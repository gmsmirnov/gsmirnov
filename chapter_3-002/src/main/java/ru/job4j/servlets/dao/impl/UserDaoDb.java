...
public class UserDaoDb implements UserDao {
    ...
    /**
     * The constant for result table column, named 'city_name'.
     */
    public static final String COLUMN_CITY_NAME = "city_name";

    /**
     * The constant for result table column, named 'region_name'.
     */
    public static final String COLUMN_REGION_NAME = "region_name";

    /**
     * The constant for result table column, named 'country_name'.
     */
    public static final String COLUMN_COUNTRY_NAME = "country_name";

    /**
     * The constant for table 'country' column, named 'name'.
     */
    public static final String COLUMN_COUNTRIES_NAMES = "name";
    ...
    public List<String> findCitiesByCountry(String country) throws DaoSystemException {
        ...
        try (Connection connection = UserDaoDb.SOURCE.getConnection();
             PreparedStatement statement = connection.prepareStatement(
                     "select city_name from country_city where country_name = ? order by (city_name);")) {
            statement.setString(1, country);
            try (ResultSet rslSet = statement.executeQuery()) {
                while (rslSet.next()) {
                    result.add(rslSet.getString(UserDaoDb.COLUMN_CITY_NAME));
                }
            }
        } catch (SQLException e) {
            throw new DaoSystemException(e.getMessage(), e);
    ...
    public List<String> findCountries() throws DaoSystemException {
        ...
        try (Connection connection = UserDaoDb.SOURCE.getConnection();
            ...
            try (ResultSet rslSet = statement.executeQuery()) {
                while (rslSet.next()) {
                    result.add(rslSet.getString(UserDaoDb.COLUMN_COUNTRIES_NAMES));
                }
            }
        } catch (SQLException e) {
            throw new DaoSystemException(e.getMessage(), e);
    ...
    // todo: release method, it is necessary for validate service
    /**
     *
     * @param country
     * @return
